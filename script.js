...
async function loadBadWordsList() {
    ...
    if (container) {
        container.innerHTML = badWordsList.map(word => `
            <span style="background: #fd1d1d20; padding: 4px 12px; border-radius: 20px; font-size: 12px;">
                ${escapeHtml(word)} 
            ...
            <div class="post-card ${isPinned ? 'pinned' : ''} fade-in">
                ...
                <div class="post-header">
                    ...
                    <div style="display: flex; gap: 12px;">
                        ...
                        <button class="post-menu" onclick="event.stopPropagation(); savePost('${post.id}')"><i class="fas fa-bookmark ${isSaved.exists() ? 'saved' : ''}" style="${isSaved.exists() ? 'color: #ffd700;' : ''}"></i></button>
...
window.openProfile = async function(userId) {
    ...
    document.getElementById('profileBio').textContent = userData.bio || "مرحباً! أنا في TOKE ✨";
...
window.openConversations = async function() {
    ...
    for (const conv of conversations) {
        ...
        html += `<div class="follower-item" onclick="closeConversations(); openChat('${conv.userId}')">
            ...
            ${unreadCount > 0 ? `<div style="background: #fd1d1d; color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 10px;">${unreadCount}</div>` : ''}
        </div>`;
    }
